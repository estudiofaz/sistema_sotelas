...
class Admin_ProdutosController extends Zend_Controller_Action
{
				...
				if ($files != null){
					...
					if($upload->isValid($file)){
						...
						//print_r($data);die;
							...
							$this->_redirect('/admin/produtos/view/id/'.$categoria_idCategoria.'/msg/1');
								
						}else{
							$this->_redirect('/admin/produtos/view/id/'.$categoria_idCategoria.'/msg/3');
						}
					}else{
    ...
    public function delAction()
    {
		...
		$modelProduto = new Admin_Model_DbTable_Produto();
		$sqlProduto = $modelProduto->select()->where('idProduto = ?', $id);
		...
		$del = $this->getRequest()->getPost('up');
			if($del == 'Excluir'){
				...
				$this->_redirect('/admin/produtos/view/id/'.$idd.'/msg/1');
			
			}elseif ($del == 'Cancelar'){
				
				$this->_redirect('/admin/produtos/view/id/'.$idd);
			}
    ...
    public function editAction(){
		...
		$modelCategoria = new Admin_Model_DbTable_Categoria();
		$sqlCategoria = $modelCategoria->select()->where('idCategoria = ?', $cat);
		$categoria = $modelCategoria->getAdapter()->fetchRow($sqlCategoria);
		$this->view->categoria = $categoria;
				...
				if ($files != null){
					...
					if($upload->isValid($file)){
						...
						$data = array(
							...
							$this->_redirect('/admin/produtos/view/id/'.$idCategoria.'/msg/2');
								
						}else{
							$this->_redirect('/admin/produtos/view/id/'.$idCategoria.'/msg/3');
						}
					}else{
