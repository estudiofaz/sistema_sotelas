...
class Admin_GrupoprimeiroController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $sqlPrincipal = $modelPrincipal->select()->where('idPrincipal = 4');
		...
		$sqlPrincipal = $modelPrincipal->select()->where('idPrincipal = 8');
    	...
    	if ($msg == 1){
	        $alert = "<div class='alert alert-success'>
	        			...
	        			Suas altera√ß√µes foram efetuadas com sucesso.
	        		</div>";
		...
		$up = $this->getRequest()->getPost('up');
			...
			if($up == 'Salvar'){		
				...
				$modelUpload = new Admin_Model_DbTable_Principal();
					...
					$where = $modelUpload->getAdapter()->quoteInto('idPrincipal= 4');
		        	$modelUpload->update($dados, $where);
			}
    	}	
				$this->_redirect('/admin/grupoprimeiro/index/msg/1');
    }
    ...
    public function group1Action()
    {
		...
		if($this->getRequest()->isPost()){
			...
			if($up == 'Salvar'){
				...
				if ($files != null){
					//FAZ LA√áO PARA INSERIR OS ARQUIVOS
					...
					if($upload->isValid($file)){
						...
						if($upload->receive($file)){
							...
							$nome = $exp[1];
							$desc1 = 'Grupo 1'; // altereaÁ„o felipe	
							$data = array(
										'tituloPrincipal'	=> 	$titulo,
										'descPrincipal'		=>	$desc1,
							...
							//print_r($data);die;
							$where = $modelUpload->getAdapter()->quoteInto('idPrincipal = 5');		
							...
							$this->_redirect('/admin/grupoprimeiro/index/msg/2');
						}
					}else{
				...
				if ($files != null){
					...
					if($upload->isValid($file)){
						...
						if($upload->receive($file)){
							...
							$nome = $exp[1];
							$desc2 = "grupo 2";// felipe alteraÁ„o	
							$data = array(
										'tituloPrincipal'	=> 	$titulo,
										'descPrincipal'		=>	$desc2,
							...
							$where = $modelUpload->getAdapter()->quoteInto('idPrincipal = 6');		
							//$modelUpload->insert($data);
							$modelUpload->update($data, $where);
				...
				if ($files != null){
					...
					if($upload->isValid($file)){
						...
						if($upload->receive($file)){
							...
							$nome = $upload->getFileName($file);
							$exp = explode($dir, $nome);
			                 $desc3 = "grupo 3"; // felipe alteraÁ„o
							$nome = $exp[1];
							...
							$where = $modelUpload->getAdapter()->quoteInto('idPrincipal = 7');		
							//$modelUpload->insert($data);
							$modelUpload->update($data, $where);
				...
				if ($files != null){
					...
					if($upload->isValid($file)){
						...
						if($upload->receive($file)){
							...
							$where = $modelUpload->getAdapter()->quoteInto('idPrincipal = 8');		
							//$modelUpload->insert($data);
							$modelUpload->update($data, $where);
